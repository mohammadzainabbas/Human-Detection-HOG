...
def Extract_Features():
    ...
    for n in range(0, len(onlyfiles)):
        ...
        feat, hog_image = hog(img[n], orientations=8, pixels_per_cell=(16, 16),cells_per_block=(4, 4), visualise=True)
...
def Detect_Human(filename, linear_svm_classifier):
    ...
    feat, hog_image = hog(img, orientations=8, pixels_per_cell=(16, 16),cells_per_block=(4, 4), visualise=True)

    feat = feat.reshape(1, -1)
    features = [np.mean(f)*np.sum(f) for f in feat]
